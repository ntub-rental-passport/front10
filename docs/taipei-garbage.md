...
參考 Yukaii/garbage 的操作概念，未搬用原始碼（檢視時未見明確 LICENSE）。白色底圖使用 OpenFreeMap Positron，橘色站點、藍色使用者定位圈；減少動態效果偏好會停用定位動畫。航照使用國土測繪中心 PHOTO2 WMTS，保留來源註記，非即時氣象影像： https://maps.nlsc.gov.tw/S09SOA/pro/Wmts_ajax_main.jsp 。

路線按行政區／分隊／路線／車號／車次分組，再依表定時間排序；相同時間的先後未確認。虛線僅為站序示意，缺少座標時斷開，不是道路路徑或真實車輛軌跡。若要沿道路呈現，須另建 Valhalla 等道路路徑產製服務並保存来源與版本；目前未部署。

回報視窗僅產生 RentMate GitHub Issues 草稿連結，由使用者自行審閱送出。未新增匿名寄信或直接寫入外部 API；需要 GitHub 帳號及儲存庫存取權，站點資料不會因回報自動改寫。維護者需在 Issues 處理查核、修正及關閉。

## 每月資料更新

`.github/workflows/update-taipei-garbage.yml` 每月 1 日臺北時間 10:17 排程，可手動觸發；需合併至 GitHub 預設分支、啟用 Actions，並允許工作流程建立 PR。GitHub 排程可能延遲，不保證準點。

`npx tsx scripts/update-taipei-garbage.ts` 僅抓取與檢查，不修改資料；加上 `--write` 才更新 CSV 與 metadata。完整讀取官方分頁、比對總數、驗證 12 行政區及所有必要欄位；重複班次、時間錯誤、筆數變動超過 20%、座標異常超過 1% 都會中止。檢查完成前不寫入檔案；部署版本只有在完整更新通過並發布後才改變。

工作流程會跑測試、建置並上傳產物，然後建立資料更新 PR。尚未連接正式部署平台，不會自動部署。平台確定後需串接發布步驟，前後端須同步同一份 CSV，後端需重新啟動以清除快取。既有提醒仍保留原班表快照，班表更改後需通知使用者重設。每月抓取不能取代颱風／假日臨時公告。

`lastCheckedAt` 為抓取檢查時間，不等同官方內容更新時間；API 未提供確切內容版本日期時 `sourceUpdatedAt` 設 null，不以抓取時間冒充官方更新日。
