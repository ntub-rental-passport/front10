...
### 診斷工作區與處理紀錄（2026-09-17.2）

2026-09-17.3：規則原文分段加入換行編號與費用小節邊界，電費證據不延伸至下一條瓦斯費；保留跨頁來源、小數及原始字元位置。右側面板改為固定標題、可收合警示與分類頁籤，僅結果清單捲動；分類支援鍵盤方向鍵及 Home／End。法規短連結另附完整名稱，押金摘要直接呈現超額數值。

工作區依「風險提醒／待確認／處理紀錄」分類，來源與完整原文收在判斷依據。統計卡可依等級篩選。頁首保留大型標題與高、中、低風險及待確認四張統計卡；法規總覽展開於工作區上方。

處理紀錄目前保存在同一瀏覽器分頁的 sessionStorage，依契約 reviewSessionId 隔離，未同步後端。記錄溝通與提出修正不會直接移除風險；返回分析頁會重新檢查儲存後的文字與欄位。使用者確認不適用必須填寫理由，證據變動後失效，也可恢復檢查。PDF 逐一輸出原文來源頁碼及處理紀錄，隱私版沿用個資遮蔽。

規則保留同規則的多處候選，明確證據可補足先前的引述候選；相關更正或相反權利約定仍需核對。解約沒收押金規則只依賴已確認的押金判定，不使用待確認的數值候選。否定範圍測試涵蓋混合保護／禁止句，並保留逗號分隔的金額及條件關係。
