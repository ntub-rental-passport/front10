...
# 長查詢要不要切窗（字元數，0 = 不切）。
#
# text2vec 的上限是 512 token。一份 3000-6000 字的合約直接丟進去，
# 它只會讀到開頭，後面全部看不到 —— 而且不報錯。
#
# 切窗之後**不合併**，由 law_corpus._rank 對每一窗各算一次相似度、取最高。
# 取平均的話，平均向量會退化成「這類文件的平均樣子」，反而配上最通用的段落。
#
# ⚠️ 更正（2026-09-13）：這段機制最早是為了解釋「local 檢索品質比 nvidia 差」
#    而加的，但那個解釋是錯的 —— check_llm.py 的範例合約去識別化後只有
#    225 字，低於門檻，從頭到尾都是 1 窗，根本沒被平均過。
#    切窗本身是對的（真實合約會用到），但它不是那次品質落差的原因。
#
