    ...
    <header class="page-header">
      ...
      <div class="header-actions">
        <button
          ...
          :title="emergencyOnly ? '顯示全部案件' : '查看緊急案件'"
          @click="toggleEmergencyFilter"
        >
          <AlertTriangle /><span>緊急案件</span><b>{{ emergencyCount }}</b>
        </button>
      </div>
    </header>
...
.page-header { @apply flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between; }.page-header h1 { @apply text-[30px] font-extrabold leading-tight tracking-tight lg:text-[32px]; }.page-header p { @apply mt-2 text-sm text-[#56635b]; }.header-actions { @apply flex flex-wrap items-center gap-2 sm:justify-end; }
...
@media (max-width:640px) { .header-actions { @apply w-full; }.header-actions > button { @apply flex-1; }.workflow-summary ol { @apply grid-cols-1 items-start gap-2; }.workflow-summary li { @apply flex-row text-left text-xs; }.field-with-action input { @apply pr-3; }.field-with-action button { @apply relative right-auto top-auto mt-2 w-full; } }
</style>
