...
const toneClasses: Record<string, string> = {
  ...
  amber: 'bg-[#fff0d8] text-[#b67a23]',
  blue: 'bg-[#e3f2f7] text-[#397a91]',
  purple: 'bg-[#eee7fb] text-[#7956a9]',
}
</script>
  ...
  <div class="mx-auto max-w-[1600px] space-y-6">
    ...
    <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <article
        v-for="item in stats"
        :key="item.title"
        class="rounded-[1.5rem] border border-[#e2ddcf] bg-white/90 p-5 shadow-[0_10px_30px_rgba(75,67,51,.05)]"
      >
        <div class="flex items-start justify-between">
          <div>
            <p class="text-sm font-semibold text-[#68736c]">{{ item.title }}</p>
            <p class="mt-2 text-2xl font-black">{{ item.value }}</p>
            <p class="mt-1 text-sm text-[#7d847f]">{{ item.note }}</p>
          </div>
          <span :class="['grid h-11 w-11 place-items-center rounded-full', toneClasses[item.tone]]"
    ...
    <section class="grid gap-5 xl:grid-cols-[minmax(0,1.65fr)_minmax(320px,.95fr)]">
      ...
      >
        <div class="flex items-center justify-between border-b border-[#e8e2d6] p-5">
          <div>
            <h2 class="text-xl font-black">本月收租進度</h2>
            <p class="mt-1 text-sm text-[#7a827c]">應收、已收與待收金額集中檢視。</p>
          </div>
          <RouterLink
            to="/landlord/finance"
            class="rounded-full border border-[#ddd6c9] px-4 py-2 text-sm font-semibold"
            >查看收款明細</RouterLink
          >
        </div>
        <div class="p-5">
          ...
          >
            ...
            <RouterLink
              to="/landlord/finance"
              class="shrink-0 rounded-full bg-white px-4 py-2 text-center text-sm font-bold shadow-sm"
              >前往收款工作台</RouterLink
            >
          </div>
        </div>
      ...
      <article class="rounded-[1.5rem] border border-[#e2ddcf] bg-white/90 shadow-sm">
        <div class="border-b border-[#e8e2d6] p-5">
          <h2 class="text-xl font-black">今日優先處理</h2>
        ...
        <div class="space-y-3 p-4">
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="truncate text-xs text-[#7a827c]">{{ item.detail }}</p>
            </div>
    ...
    <section class="grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)]">
      ...
      <article class="rounded-[1.5rem] border border-[#e2ddcf] bg-white/90 p-5 shadow-sm">
        <h2 class="text-xl font-black">快速操作</h2>
        <p class="mt-1 text-sm text-[#7a827c]">常用功能快速入口。</p>
        <div class="mt-5 grid grid-cols-2 gap-3">
          <RouterLink
            to="/landlord/tenants"
            class="rounded-2xl border border-[#e3ddcf] p-4 font-bold hover:bg-[#f4f7f1]"
            ><Plus class="mb-3 h-5 w-5 text-[#5b8263]" />新增租客</RouterLink
          ><RouterLink
            to="/landlord/contracts"
            class="rounded-2xl border border-[#e3ddcf] p-4 font-bold hover:bg-[#f4f7f1]"
            ><FileText class="mb-3 h-5 w-5 text-[#5b8263]" />新增合約</RouterLink
          ><RouterLink
            to="/landlord/properties"
            class="rounded-2xl border border-[#e3ddcf] p-4 font-bold hover:bg-[#f4f7f1]"
            ><Building2 class="mb-3 h-5 w-5 text-[#5b8263]" />新增房源</RouterLink
          ><RouterLink
            to="/landlord/maintenance"
            class="rounded-2xl border border-[#e3ddcf] p-4 font-bold hover:bg-[#f4f7f1]"
            ><Wrench class="mb-3 h-5 w-5 text-[#5b8263]" />新增報修</RouterLink
          >
        </div>
      </article>
    </section>
