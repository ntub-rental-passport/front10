...
const lawRows = [
  {
    ...
    point: '任意終止、通知與違約金',
    usage:
      '租約約定得終止時，至少提前一個月通知；未先期通知而逕行終止，違約金最高不得超過一個月租金。',
  ...
  },
  {
    ...
    point: '公告應記載事項的效力',
    usage:
      '企業經營者使用的定型化契約違反公告事項時，相關條款可能無效；漏載的應記載事項仍構成契約內容。',
...
<template>
  <article class="min-h-full overflow-x-clip bg-[#f7f5ef] pb-10 text-slate-900">
    <TenantGuideHeader active-topic="penalty" />
    ...
    <div class="mx-auto flex w-full max-w-[1540px] flex-col gap-6 px-4 py-5 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between gap-3">
        <Button
          as-child
          variant="ghost"
          class="rounded-full px-0 text-slate-600 hover:bg-transparent hover:text-primary"
        >
          <RouterLink to="/app">
        ...
        <Badge
          variant="outline"
          class="rounded-full border-indigo-200 bg-white px-3 py-1 text-indigo-700"
        >
          租客防禦指南
        </Badge>
      </div>
      ...
      <section class="grid gap-5 lg:grid-cols-[minmax(0,1.04fr)_minmax(360px,0.64fr)]">
        <Card class="overflow-hidden rounded-[2rem] border-0 bg-white shadow-sm">
          <CardContent
            class="grid gap-6 p-5 md:grid-cols-[minmax(0,0.95fr)_minmax(300px,0.8fr)] md:p-8"
          >
            <div class="flex flex-col justify-center gap-5">
              <div class="flex flex-wrap gap-2">
                <Badge class="rounded-full bg-indigo-600 px-3 py-1 text-white hover:bg-indigo-600"
                  >違約金上限解析</Badge
                >
                <Badge
                  variant="outline"
              ...
              <div class="space-y-4">
                <h1
                  class="max-w-2xl text-balance font-serif text-3xl font-black leading-[1.15] tracking-tight text-slate-950 sm:text-4xl lg:text-[3.25rem]"
                >
              ...
              <div class="grid gap-2 sm:grid-cols-3">
                <div class="min-w-0 rounded-2xl bg-indigo-50 px-3 py-4">
                  ...
                  <p class="mt-1 whitespace-nowrap text-base font-black leading-6 text-indigo-950">
                    通常 0 元
                  </p>
                </div>
                <div class="min-w-0 rounded-2xl bg-rose-50 px-3 py-4">
                  <p class="whitespace-nowrap text-xs font-semibold text-rose-500">未按期通知</p>
                  <p class="mt-1 whitespace-nowrap text-base font-black leading-6 text-rose-950">
                    最高 1 個月
                  </p>
                </div>
                <div class="min-w-0 rounded-2xl bg-emerald-50 px-3 py-4">
                  ...
                  <p class="mt-1 whitespace-nowrap text-base font-black leading-6 text-emerald-950">
                    提前留證
                  </p>
                </div>
              </div>
        ...
        <Card class="rounded-[2rem] border border-slate-200 bg-white shadow-sm">
          <CardContent class="flex h-full flex-col gap-5 p-6">
            ...
            <label
              class="flex cursor-pointer items-start gap-3 rounded-2xl border border-indigo-100 bg-indigo-50/70 p-4 text-sm text-indigo-950"
            >
              <input
              ...
              <span>
                ...
                <span class="mt-1 block leading-6 text-indigo-800/80"
                  >此試算假設租約已勾選「得任意終止」。</span
                >
              </span>
            </label>

            <div
              class="mt-auto space-y-3 rounded-3xl border border-indigo-100 bg-gradient-to-br from-indigo-50 via-white to-violet-50 p-5 text-slate-900 shadow-[inset_0_1px_0_rgba(255,255,255,0.9)]"
            >
              <div class="flex items-center justify-between gap-4 text-sm text-slate-500">
                <span>房東要求</span>
                <strong class="text-base tabular-nums text-slate-900">{{
                  formatCurrency(landlordRequest)
                }}</strong>
              </div>
              <div class="flex items-center justify-between gap-4 text-sm text-slate-500">
                <span>{{ legalCapLabel }}</span>
                <strong class="text-base tabular-nums text-emerald-700">{{
                  formatCurrency(legalCap)
                }}</strong>
              </div>
              <div class="h-px bg-indigo-100" />
              <div class="flex items-center justify-between gap-4">
                <span class="text-sm font-semibold text-slate-600">可能超收</span>
                <strong class="text-2xl tabular-nums text-rose-600">{{
                  formatCurrency(overLimitAmount)
                }}</strong>
              </div>
            </div>
          </CardContent>
        ...
        <div class="flex w-full max-w-[860px] flex-col gap-10 pt-2">
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm text-slate-500">
                  資料來源：法律圈 LawChain、律果 AI 法律事務所整理
                </p>
              </div>
            </div>

            <blockquote
              class="border-l-4 border-violet-300 bg-violet-50/60 py-5 pl-5 pr-4 text-base leading-8 text-violet-950 sm:text-lg"
            >
            ...
            <div class="space-y-3">
              <div class="flex gap-3 rounded-2xl border border-slate-100 bg-white p-4">
                ...
                <p class="text-sm leading-6 text-slate-600">
                  若「違約處理費」沒有具體對價，只是因提前解約而收取，可能會被視為違約金。
                </p>
              </div>
              <div class="flex gap-3 rounded-2xl border border-slate-100 bg-white p-4">
                <CheckCircle2 aria-hidden="true" class="mt-0.5 h-5 w-5 shrink-0 text-emerald-500" />
                <p class="text-sm leading-6 text-slate-600">
                  若租約允許任意終止且承租人已依約先行通知，應先主張不發生未通知違約金，而非直接接受一個月上限。
                </p>
              </div>
            </div>
          </section>
          ...
          <section class="space-y-5">
            ...
            >
              ...
              <p>
                <strong>法條校正：</strong>《租賃住宅市場發展及管理條例》第 7 條規範的是押金不得超過
                2 個月租金，不是提前終止違約金上限；本頁改以定型化契約應記載事項第 14 點說明。
              </p>
            </div>
          </section>
            ...
            >
              <div class="overflow-hidden rounded-3xl bg-[#faf8f1] sm:w-[220px] sm:shrink-0">
                <img
                  :src="section.image"
                  :alt="section.title"
                  class="h-full max-h-[220px] w-full object-cover"
                  width="1254"
                  height="1254"
                  loading="lazy"
                />
          ...
          <!-- 回覆房東範本 -->
          <section
            class="overflow-hidden rounded-[2rem] bg-gradient-to-br from-indigo-700 via-indigo-600 to-violet-600 p-6 text-white sm:p-8"
          >
            <div class="flex items-center gap-4">
              ...
              <h2 class="min-w-0 text-balance text-xl font-black sm:text-2xl">
                可以直接放進回覆房東的句子
              </h2>
            </div>
          ...
          <section class="space-y-5">
            ...
            <div
              class="divide-y divide-slate-200 overflow-hidden rounded-3xl border border-slate-200 bg-white"
            >
              <div v-for="(item, index) in qaItems" :key="item.q">
                ...
                >
                  <span class="min-w-0 flex-1 text-sm font-bold text-slate-900 sm:text-base">{{
                    item.q
                  }}</span>
                  <ChevronDown
                    aria-hidden="true"
          ...
          <!-- 分享文章 -->
          <section
            class="flex flex-col items-start justify-between gap-4 rounded-3xl border border-slate-200 bg-white p-5 sm:flex-row sm:items-center sm:p-6"
          >
            <div>
              <p class="text-base font-black text-slate-900">分享這篇文章</p>
              <p class="mt-1 text-sm text-slate-500">
                覺得有幫助嗎？複製連結分享給正在煩惱租約的朋友。
              </p>
            </div>
            <button
              type="button"
