...
export function useNotesState(mode: MainTab) {
  ...
  async function loadPersonalNotes(): Promise<void> {
    try {
      personalNotes.value = (await listPersonalNotes()) as PersonalNote[]
    } catch {
      // The local seed remains visible if the API or authentication is unavailable.
    }
  }
