...
watch(activeTab, (tab) => {
  nextTick(() => {
    if (tab === 'map') mainMap ? mainMap.resize() : startMap('main')
    else if (tab === 'nearby') nearbyMap ? nearbyMap.resize() : startMap('nearby')
    else if (tab === 'manual') manualMap ? manualMap.resize() : startMap('manual')
  })
})
