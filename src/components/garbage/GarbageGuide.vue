...
const steps = [
  {
    ...
    icon: Search,
    title: '用地址，找到清運班次',
    text: '先選擇臺北市或新北市，再選行政區與里別、道路、日期與時間。地圖同步顯示符合條件的站點；點選橘色小點查看詳情，各站點不會聚合成數字圓圈。',
  ...
  },
  {
    ...
    icon: List,
    title: '用列表，比較時間與路線',
    text: '可切換臺北市、新北市。每頁顯示 20 筆班次，依條件縮小結果。新北市依各站每週垃圾、回收與廚餘班表篩選。相同地址可能有不同車次，收藏或提醒前請確認抵達時間。',
    labels: ['選擇縣市', '篩選班次', '比較表定時間'],
  },
  {
    tab: 'nearby' as const,
    icon: Crosshair,
    title: '打開 GPS，選擇附近查詢範圍',
    text: '按「啟用 GPS 定位」並允許位置權限。系統會持續更新你的所在位置，可選擇半徑 100、200、300、400 或 500 公尺，將範圍內的班表站點依直線距離排序。若沒有結果，可改用手動選點。',
    labels: ['允許定位', '100～500 m 範圍', '依距離排序'],
  },
  {
    tab: 'manual' as const,
    icon: MapPin,
    title: '點一下，先查下一個目的地',
    text: '切換手動定位，在地圖空白處點選目的地。橘色定位點是查詢中心，虛線圈代表半徑 200 公尺範圍；再次點選即可換位置，附近清運地點看板也會同步更新。無需開啟 GPS。',
  ...
  <section class="garbage-guide">
    ...
    <div class="panel guide-faq">
      ...
      <details>
        ...
        <p>
          點選地圖上的「路線列表」，選擇路線、車次與車號，即可查看各站表定時間。藍色霓虹實線為站序連線示意，不是道路導航，也不代表垃圾車目前行進位置。可按「隱藏路線」暫時隱藏連線與編號，再按「顯示路線」恢復；關閉路線卡會一併清除路線。「航照影像」可切換國土測繪中心底圖；不是即時氣象雲圖。
        </p>
      </details>
      <details>
