...
const EMPTY_CANDIDATE: ContractFieldCandidate = {
  value: '',
  sourceValue: '',
  confidence: 'low',
}

const FINANCIAL_DIGITS: Record<string, number> = {
  零: 0,
  〇: 0,
  '○': 0,
  一: 1,
...
function candidate(
  ...
  return {
    value: cleanSource(value),
    sourceValue: cleanSource(sourceValue),
    confidence,
  }
}
...
function captureFirst(text: string, patterns: RegExp[]): string {
  for (const pattern of patterns) {
    ...
    const value = cleanSource(match?.[1])
    if (value) return value
  }
  return ''
}

function normalizePersonName(rawValue: string): string {
  ...
  if (!/^[\p{Script=Han}·‧]{2,20}$/u.test(value)) return ''
  return value
}

function extractPerson(text: string, role: '出租人' | '承租人'): ContractFieldCandidate {
  ...
  if (/遮蔽/.test(rawValue)) {
    return candidate('影像遮蔽，請人工輸入', rawValue, 'low')
  }
  const value = normalizePersonName(rawValue)
  return value ? candidate(value, rawValue, 'medium') : { ...EMPTY_CANDIDATE }
}

function parseChineseInteger(rawValue: string): number | null {
  const normalized = normalizeFullWidthDigits(rawValue).replace(/[\s,，]/g, '')
  if (!normalized) return null
  if (/^\d+$/.test(normalized)) return Number(normalized)

  if (!/[十拾百佰千仟萬万億亿]/.test(normalized)) {
    const digitSequence = [...normalized].map((character) => FINANCIAL_DIGITS[character])
    return digitSequence.every((digit) => digit !== undefined)
      ? Number(digitSequence.join(''))
      : null
  }
  ...
  for (const character of normalized) {
    ...
    if (FINANCIAL_DIGITS[character] !== undefined) {
      digit = FINANCIAL_DIGITS[character]
      continue
    }
    if (SMALL_UNITS[character]) {
      section += (digit || 1) * SMALL_UNITS[character]
      digit = 0
      continue
    }
    if (character === '萬' || character === '万') {
      section += digit
...
function extractMoney(text: string, contextPatterns: string[]): ContractFieldCandidate {
  for (const contextPattern of contextPatterns) {
    ...
    if (amount === null || amount <= 0 || amount > 100000000) continue
    return candidate(formatMoney(amount), rawValue, 'medium')
  }
  return { ...EMPTY_CANDIDATE }
}

function extractAddress(text: string): ContractFieldCandidate {
  const rawValue = captureFirst(text, [
    /(?:甲方)?房屋所在地及使用範圍\s*[：:]?\s*([^\r\n。]{3,100})/,
  ...
  const administrativeArea = rawValue.match(
    /([\p{Script=Han}]{1,6}[縣市][\p{Script=Han}]{1,8}(?:鄉|鎮|市|區)[\p{Script=Han}]{1,8}(?:村|里)?)/u,
  )?.[1]
  const value =
    administrativeArea && rawValue.length <= administrativeArea.length + 2
      ? `${administrativeArea}［地址不完整，後段待確認］`
      : rawValue

  return candidate(value, rawValue, 'medium')
}

function formatRocDate(yearText: string, monthText: string, dayText?: string): string {
...
function extractDates(text: string): Pick<ContractFieldCandidates, 'startDate' | 'endDate'> {
...
function extractDueDay(text: string): ContractFieldCandidate {
  const monthEndMatch = text.match(/(?:租金應於\s*)?(每月(?:底|月)\s*(?:以)?前)(?:繳納)?/)
  if (monthEndMatch?.[1]) {
    return candidate('每月底前', monthEndMatch[1], 'medium')
  }

  const numberedDayMatch = text.match(/(?:租金\s*)?每月\s*([0-9０-９]{1,2})\s*日\s*前/)
  if (!numberedDayMatch?.[1]) return { ...EMPTY_CANDIDATE }
  const day = parseChineseInteger(numberedDayMatch[1])
  if (!day || day > 31) return { ...EMPTY_CANDIDATE }
  return candidate(`每月 ${day} 日前`, numberedDayMatch[0], 'medium')
}

function extractPenalty(text: string): ContractFieldCandidate {
  ...
  const movingExpense = /甲方[^\r\n。；]{0,140}?(?:搬遷|搬家)費用/.test(text)
  const summaries: string[] = []
  if (tenantMatch) summaries.push('乙方提前終止：1 個月租金')
  if (landlordMatch) {
    summaries.push(`甲方提前解約：1 個月租金${movingExpense ? '及搬遷費用' : ''}`)
  }

  if (summaries.length) {
    return candidate(
      summaries.join('；'),
      [tenantMatch?.[1], landlordMatch?.[1]].filter(Boolean).join('；'),
      summaries.length === 2 ? 'medium' : 'low',
    )
  }

  const genericMatch = text.match(/((?:支付相當於|賠償)[^\r\n。；]{0,40}?一個月租金)/)
  return genericMatch?.[1]
    ? candidate('1 個月租金', genericMatch[1], 'low')
    : { ...EMPTY_CANDIDATE }
}
